...
async function askSmolRouter(task) {
  const prompt = `Given this task, which model should handle it and why?\n\nTask: ${task}\n\nRespond with only the model name (e.g., gpt-4, claude, smollm3, perplexity) and a short reason.`;
  ...
  if (typeof response === 'string') {
    const match = response.match(/^(gpt-4|claude|smollm3|perplexity)\s*[:\-]?\s*(.*)$/i);
    if (match) {
      model = match[1].toLowerCase();
      reason = match[2] ? match[2].trim() : reason;
    } else {
      // fallback: look for model name anywhere in response
      const found = (response.match(/gpt-4|claude|smollm3|perplexity/i) || [])[0];
